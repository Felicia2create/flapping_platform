...
def generate_launch_description():
    ...
    # Launch turntable velocity controller
    start_turntable_velocity_controller_cmd = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller', '--set-state', 'active',
    ...
    # 1. Start plate controller after joint state broadcaster
    load_plate_controller_cmd = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=start_joint_state_broadcaster_cmd,
            on_exit=[start_arm_controller_cmd]))
    
    load_turntable_velocity_controller_cmd = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=start_arm_controller_cmd,
            on_exit=[start_turntable_velocity_controller_cmd]))
    ...
    ld.add_action(load_plate_controller_cmd)
    ld.add_action(load_turntable_velocity_controller_cmd)
