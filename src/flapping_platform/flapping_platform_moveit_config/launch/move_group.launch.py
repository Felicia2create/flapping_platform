#!/usr/bin/env python3
"""
Launch MoveIt 2 for the flapping_platform robotic arm.

This script creates a ROS 2 launch file that starts the necessary nodes and services
for controlling a flapping_platform robotic arm using MoveIt 2. It loads configuration files,
...
def generate_launch_description():
    """
    Generate a launch description for MoveIt 2 with flapping_platform robot.

    This function sets up the necessary configuration and nodes to launch MoveIt 2
    for controlling a flapping_platform robotic arm. It includes setting up paths to config files,
    ...
    package_name_moveit_config = 'flapping_platform_moveit_config'
    ...
    declare_robot_name_cmd = DeclareLaunchArgument(
        name='robot_name',
        default_value='flapping_platform',
    ...
    def configure_setup(context):
        ...
        stomp_config_path = os.path.join(config_path, 'stomp_planning.yaml')
        ...
        start_rviz_node_cmd = Node(
            ...
            arguments=[
                "-d",
                [FindPackageShare(rviz_config_package), "/config/", "/flapping_platform/", rviz_config_file]
            ],
            output="screen",
            parameters=[
